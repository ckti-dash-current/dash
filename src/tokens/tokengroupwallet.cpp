    ...
    CAmount totalXDMNeeded, CTokenGroupID grpID, CWallet *wallet)
{
    ...
    {
        totalWgrAvailable += coin.GetValue();
...

    {
        ...
        if (totalWgrAvailable < totalWgrNeeded + fee) // need to find a fee input
        {
            // find a fee input
        ...
        if (totalWgrAvailable > totalWgrNeeded + (FEE_FUDGE * fee))
        {
            ...
            CTxOut txout(totalWgrAvailable - totalWgrNeeded - fee, GetScriptForDestination(newKey.GetID()));
            tx.vout.push_back(txout);
        }
...
void GroupMelt(CTransactionRef &txNew, const CTokenGroupID &grpID, CAmount totalNeeded, CWallet *wallet)
{
    ...
    ConstructTx(txNew, chosenCoins, outputs, totalWgrNeeded, totalNeeded, 0, grpID,
        wallet);
