...
#include "zwagerr/accumulators.h"
#include "zwagerr/deterministicmint.h"
...
/*
    ...
    if (!pwallet->CreateZWAGERROutPut(denomStaked, outReward, dMint))
    ...
    while (toMint >= 1 * COIN) {
        ...
        if (!pwallet->CreateZWAGERROutPut(denomination, out, dMintReward))
            return error("%s: failed to create zWAGERR output", __func__);
...
{
    CxWAGERRTracker* zwagerrTracker = pwallet->zwagerrTracker.get();
    CMintMeta meta;
    if (!zwagerrTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zwagerrTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
*/
