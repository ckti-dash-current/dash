...
extern UniValue tokeninfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "\n" +
            HelpExampleCli("tokeninfo", "ticker \"WGR\"") +
            "\n"
        );
...
UniValue createrawtokentransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "      ,...\n"
            "    }\n"
            "3. \"token_outputs\"         (string, required) a json object with addresses as keys and a json objects with the WGR and tokens to send\n"
            "    {\n"
            "      \"address\":           (numeric, required) The key is the Wgr address, the value is a json object with an WGR amount, tokengroup ID and token value as values\n"
            "      {\n"
            "        \"amount\":\"x.xxx\"       (numeric, required) The WGR amount\n"
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3) {
        throw std::runtime_error(
            ...
            + HelpExampleCli("createrawtokendocument",
                "\"{\\\"ticker\\\": \\\"MGT\\\", \\\"name\\\": \\\"Management Token\\\", \\\"chain\\\": \\\"WAGERR.testnet\\\", "
                "\\\"summary\\\": \\\"The MGT token is a tokenized management key on the WAGERR blockchain with special authorities "
                ...
                "Protocol (ATP) introduces cross-coin and cross-token policy. WAGERR utilizes ATP for its reward system and rights "
                ...
                "to play a role in the management of and access to special features.\\\", \\\"creator\\\": \\\"The WAGERR Core "
                "Developers\\\", \\\"contact\\\":{\\\"url\\\":\\\"https://github.com/wagerr/wagerr\\\"}}\"") +
