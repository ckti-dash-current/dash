...
std::vector<uint8_t> PackAddrData(const T &id, uint8_t type)
{
    ...
    default:
        if (type == GROUP_TYPE) // Groups can be any length
            ...
            throw std::runtime_error("Error packing wgraddr: invalid address length");
    }
...
// Implements encoding of CTxDestination using wgraddr.
class WgrAddrEncoder : public boost::static_visitor<std::string>
{
public:
    WgrAddrEncoder(const CChainParams &p) : params(p) {}
    std::string operator()(const CKeyID &id) const
    {
        std::vector<uint8_t> data = PackAddrData(id, PUBKEY_TYPE);
        return bech32::Encode(params.GetConsensus().WgrAddrPrefix, data);
    }
...
{
    WgrAddrContent content = DecodeWgrAddrContent(addr, params);
    if (content.hash.size() == 0)
    {
        return CNoDestination{};
    }

    return DecodeWgrAddrDestination(content);
}
...
WgrAddrContent DecodeWgrAddrContent(const std::string &addr, const CChainParams &params)
{
    ...
    if (prefix != params.GetConsensus().WgrAddrPrefix)
    {
        return {};
    }
    ...
    auto type = WgrAddrType((version >> 3) & 0x1f);
...
// PackWgrAddrContent allows for testing PackAddrData in unittests due to
