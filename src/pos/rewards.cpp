...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue, bool isZWAGERRStake, const Consensus::Params& consensusParams) {
    if (nHeight >= consensusParams.V16DeploymentHeight) return blockValue > 2440 * COIN ? 2440 * COIN : 0;
    if (nHeight >= consensusParams.nBlockZerocoinV2 && !isZWAGERRStake) return blockValue > 2440 * COIN ? 2440 * COIN : 0;
    if (nHeight >= consensusParams.nBlockZerocoinV2 && isZWAGERRStake) return blockValue > 2410 * COIN ? 2410 * COIN : 0;
