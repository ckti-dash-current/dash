...
CAmount GetBlockSubsidyWgr(const int nPrevHeight, const bool fPos, const Consensus::Params& consensusParams)
{
    CAmount nSubsidy = 0;
...
CBlockReward::CBlockReward(const int nHeight, const CAmount nFees, const bool fPos, const Consensus::Params& consensusParams) {
    rewards.clear();
    fBurnUnpaidMasternodeReward = false;
    CAmount nBlockValue = GetBlockSubsidyWgr(nHeight - 1, fPos, consensusParams);
