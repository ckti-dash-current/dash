...
extern UniValue gettokenbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "\n"
            "\nExamples:\n" +
            HelpExampleCli("gettokenbalance", "groupid wgrreg1zwm0kzlyptdmwy3849fd6z5epesnjkruqlwlv02u7y6ymf75nk4qs6u85re") +
            "\n"
        );
...
extern UniValue listtokenssinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "WGR. This is negative for the 'send' category, and for the 'move' category for moves \n"
            ...
            HelpExampleCli("listtokenssinceblock", "\"wgrreg1zwm0kzlyptdmwy3849fd6z5epesnjkruqlwlv02u7y6ymf75nk4qs6u85re\" \"36507bf934ffeb556b4140a8d57750954ad4c3c3cd8abad3b8a7fd293ae6e93b\" 6") +
            HelpExampleRpc(
...
extern UniValue configuretoken(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
     ...
     if (request.fHelp || request.params.size() < 5)
        throw std::runtime_error(
            ...
            "\n"
            "\nExamples:\n" +
            HelpExampleCli("configuretoken", "\"MGT\" \"ManagementToken\" 6 \"https://raw.githubusercontent.com/wagerr/ATP-descriptions/master/WGR-mainnet-MGT.json\" 4f92d91db24bb0b8ca24a2ec86c4b012ccdc4b2e9d659c2079f5cc358413a765 true") +
            "\n"
        );
    ...
    CTransactionRef tx;
    ConstructTx(tx, chosenCoins, outputs, 0, 0, XDMFeeNeeded, grpID, pwallet);
...
extern UniValue configuremanagementtoken(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
     ...
     if (request.fHelp || request.params.size() < 5)
        throw std::runtime_error(
            ...
            "\n"
            "\nExamples:\n" +
            HelpExampleCli("configuremanagementtoken", "\"MAGIC\" \"MagicToken\" 4 \"https://raw.githubusercontent.com/wagerr/ATP-descriptions/master/WGR-testnet-MAGIC.json\" 4f92d91db24bb0b8ca24a2ec86c4b012ccdc4b2e9d659c2079f5cc358413a765 true") +
            "\n"
        );
    ...
    if (confirmed) {
        CTransactionRef tx;
        ConstructTx(tx, chosenCoins, outputs, 0, 0, 0, grpID, pwallet);
        authKeyReservation.KeepKey();
        ret.push_back(Pair("groupID", EncodeTokenGroup(grpID)));
        ret.push_back(Pair("transaction", tx->GetHash().GetHex()));
    }
...
extern UniValue createtokenauthorities(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1)
        throw std::runtime_error(
            ...
            HelpExampleCli("createtokenauthorities", "\"wgrreg1zwm0kzlyptdmwy3849fd6z5epesnjkruqlwlv02u7y6ymf75nk4qs6u85re\" \"g74Uz39YSNBB3DouQdH1UokcFT5qDWBMfa\" \"melt child\"") +
            "\n"
        );
    ...
    totalWgrNeeded += RenewAuthority(chosenCoins[0], outputs, renewAuthorityKey);

    { // Construct the new authority
        CScript script = GetScriptForDestination(dst, grpID, (CAmount)auth);
        CRecipient recipient = {script, GROUPED_SATOSHI_AMT, false};
        outputs.push_back(recipient);
        totalWgrNeeded += GROUPED_SATOSHI_AMT;
    }

    CTransactionRef tx;
    ConstructTx(tx, chosenCoins, outputs, totalWgrNeeded, 0, 0, grpID, pwallet);
...
extern UniValue listtokenauthorities(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\n"
            "\nExamples:\n"
            "\nList all available token authorities of group wgrreg1zwm0kzlyptdmwy3849fd6z5epesnjkruqlwlv02u7y6ymf75nk4qs6u85re:\n" +
            HelpExampleCli("listtokenauthorities", "\"wgrreg1zwm0kzlyptdmwy3849fd6z5epesnjkruqlwlv02u7y6ymf75nk4qs6u85re\" ") +
            "\n"
        );
...
extern UniValue droptokenauthorities(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1)
        throw std::runtime_error(
            ...
            HelpExampleCli("droptokenauthorities", "\"wgrreg1zwm0kzlyptdmwy3849fd6z5epesnjkruqlwlv02u7y6ymf75nk4qs6u85re\" \"a018c9581b853e6387cf263fc14eeae07158e8e2ae47ce7434fcb87a3b75e7bf\" 1 \"mint\" \"melt\"") +
            "\n"
        );
...
extern UniValue minttoken(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "\n"
            "\nExample:\n" +
            HelpExampleCli("minttoken", "wgrreg1zwm0kzlyptdmwy3849fd6z5epesnjkruqlwlv02u7y6ymf75nk4qs6u85re gMngqs6eX1dUd8dKdwPqGJchc1S3e6b9Cx 40") +
            "\n"
        );
    ...
    CTokenGroupID grpID;
    CAmount totalTokensNeeded = 0;
    CAmount totalWgrNeeded = GROUPED_SATOSHI_AMT; // for the mint destination output
    unsigned int curparam = 0;
    std::vector<CRecipient> outputs;
    ...
    coinControl.fAllowOtherInputs = true; // Allow a normal WGR input for change
    std::string strError;
    ...
    totalWgrNeeded += RenewAuthority(authority, outputs, childAuthorityKey);
    ...
    CTransactionRef tx;
    ConstructTx(tx, chosenCoins, outputs, totalWgrNeeded, 0, XDMFeeNeeded, grpID, pwallet);
...
extern UniValue melttoken(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "\n"
            "\nExample:\n" +
            HelpExampleCli("melttoken", "wgrreg1zwm0kzlyptdmwy3849fd6z5epesnjkruqlwlv02u7y6ymf75nk4qs6u85re 4.3") +
            "\n"
        );
