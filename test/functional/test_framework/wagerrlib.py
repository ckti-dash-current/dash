...
def init(libwgrcorefile=None):
    global wgrlib
    if libwgrcorefile is None:
        libwgrcorefile = "libwgrcore.so"
        try:
            wgrlib = CDLL(libwgrcorefile)
        except OSError:
            import os
            dir_path = os.path.dirname(os.path.realpath(__file__))
            wgrlib = CDLL(dir_path + os.sep + libwgrcorefile)
    else:
        wgrlib = CDLL(libwgrcorefile)
    if wgrlib is None:
        raise Error("Cannot find %s shared library", libwgrcorefile)
...
def bin2hex(data):
    ...
    assert type(data) is bytes, "wgrlib.bintohex requires parameter of type bytes"
    l = len(data)
    result = create_string_buffer(2 * l + 1)
    if wgrlib.Bin2Hex(data, l, result, 2 * l + 1):
        return result.value.decode("utf-8")
    raise Error("wgrlib bin2hex error")
...
def signTxInput(tx, inputIdx, inputAmount, prevoutScript, key, sigHashType=SIGHASH_FORKID | SIGHASH_ALL):
    ...
    """
    assert (sigHashType & SIGHASH_FORKID) > 0, "Did you forget to indicate the coin hashing algorithm?"
    if type(tx) == str:
        tx = unhexlify(tx)
    elif type(tx) != bytes:
        tx = tx.serialize()
    ...
    siglen = wgrlib.SignTx(tx, len(tx), inputIdx, c_longlong(inputAmount), prevoutScript,
                            len(prevoutScript), sigHashType, key, result, 100)
    if siglen == 0:
        raise Error("wgrlib signtx error")
...
def randombytes(length):
    ...
    worked = wgrlib.RandomBytes(result, length)
    if worked != length:
        raise Error("wgrlib randombytes error")
    return result.value
...
def pubkey(key):
    ...
    l = wgrlib.GetPubKey(key, result, 65)
...
def txid(txbin):
    """Return a transaction id, given a transaction in hex, object or binary form.
       The returned binary txid is not reversed.  Do: hexlify(wgrlib.txid(txhex)[::-1]).decode("utf-8") to convert to
       wagerrd's hex format.
    """
    if type(txbin) == str:
