...
extern UniValue listtokentransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"Wgr address\",    (string) The Wgr address of the transaction. Not present for \n"
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
     ...
     if (request.fHelp || request.params.size() < 5)
        throw std::runtime_error(
            ...
            "\n"
            "\nExamples:\n" +
            HelpExampleCli("configuretoken", "\"MGT\" \"ManagementToken\" 6 \"https://raw.githubusercontent.com/wagerrATP-descriptions/master/WGR-mainnet-MGT.json\" 4f92d91db24bb0b8ca24a2ec86c4b012ccdc4b2e9d659c2079f5cc358413a765 true") +
            "\n"
        );
...
extern UniValue configuremanagementtoken(const JSONRPCRequest& request)
{
    ...
    // Otherwise: spend a Wgr output from the token management address
...
extern UniValue createtokenauthorities(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1)
        throw std::runtime_error(
            "createtokenauthorities \"groupid\" \"wgraddress\" authoritylist \n"
...
extern UniValue minttoken(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1)
        throw std::runtime_error(
            "minttoken \"groupid\" \"wgraddress\" quantity \n"
            "\nMint new tokens.\n"
            "\nArguments:\n"
            "1. \"groupID\"     (string, required) the group identifier\n"
            "2. \"address\"     (string, required) the destination address\n"
