...
{
    return GetBlockSubsidyWgr(nPrevHeight, fPos, consensusParams);
}
...
static int64_t nTimeWgrSpecific = 0;
...
*/
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeWgrSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Wagerr specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeWgrSpecific * MICRO, nTimeWgrSpecific * MILLI / nBlocksTotal);
