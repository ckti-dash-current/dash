...
extern UniValue gettokenbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "2. \"address\" (string, optional) the WGR address to filter\n"
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
     ...
     if (request.fHelp || request.params.size() < 5)
        throw std::runtime_error(
            ...
            "\n"
            "\nExamples:\n" +
            HelpExampleCli("configuremanagementtoken", "\"GVT\" \"GuardianValidatorToken\" 4 \"https://raw.githubusercontent.com/wagerr/ATP-descriptions/master/WGR-testnet-MGT.json\" 4f92d91db24bb0b8ca24a2ec86c4b012ccdc4b2e9d659c2079f5cc358413a765 true") +
            "\n"
        );
...
UniValue listunspenttokens(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "4. \"addresses\"      (string) A json array of WGR addresses to filter\n"
            "    [\n"
            "      \"address\"     (string) WGR address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the WGR address\n"
            "    \"address\" : \"address\",    (string) the WGR address\n"
            "    \"address\" : \"address\",    (string) the WGR address\n"
