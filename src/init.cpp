...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-wgrstake=<n>", strprintf(_("Enable or disable staking functionality for WAGERR inputs (0-1, default: %u)"), 1), false, OptionsCategory::BLOCK_CREATION);
...
bool AppInitMain()
{
    ...
    if (!HasWallets() || wallets.size() < 1) {
        ...
        stakingManager->fEnableWGRStaking = false;
    } else {
        ...
        stakingManager->fEnableWGRStaking = gArgs.GetBoolArg("-staking", true);
    }
