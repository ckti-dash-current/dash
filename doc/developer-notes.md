...
- [Development guidelines](#development-guidelines)
    - [General Bytz Core](#general-bytz-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Bytz Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/bytzcurrency/bytz/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bytz
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bytz --log_level=test_suite
$ valgrind -v --leak-check=full src/bytzd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_bytz.coverage/index.html`.
```

**Sanitizers**

Bytz Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 55002.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 55003 for connections and services them.
...
be sent upstream but bugfixes may also be prudent to PR against Bytz Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bytz-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bytz-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Bytz Core, the wallet was always
